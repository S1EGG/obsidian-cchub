...
export function useSessionLifecycle(
	...
	/**
	 * Create a new session with the active agent (or override).
	 */
	const createSession = useCallback(
		async (agentIdOverride?: string) => {
			const settings = settingsAccess.getSnapshot();
			const activeAgentId = agentIdOverride ?? getActiveAgentId(settings);
			...
			try {
				...
				console.debug(
					"[useSessionLifecycle] agentConfig:",
					agentConfig,
				);
				...
				const needsInitialize =
					!isInit || currentAgentId !== activeAgentId;
				...
				if (needsInitialize) {
					console.debug(
						"[useSessionLifecycle] Calling initialize...",
					);
	...
	const restartSession = useCallback(
		async (agentIdOverride?: string) => {
			await createSession(agentIdOverride);
		},
		[createSession],
	);
