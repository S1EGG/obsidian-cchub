...
export function ChatInput({
		...
		async (files: File[]) => {
			...
			for (const file of files) {
				...
				if (attachedImages.length + addedCount >= MAX_IMAGE_COUNT) {
					new Notice(
						`[CCHub] Maximum ${MAX_IMAGE_COUNT} images allowed`,
					);
					break;
				}
				...
				if (file.size > MAX_IMAGE_SIZE_BYTES) {
					new Notice(
						`[CCHub] Image too large (max ${MAX_IMAGE_SIZE_MB}MB)`,
					);
					continue;
				}
	...
	const handleInsertMentionTrigger = useCallback(() => {
		...
		const newText = `${inputValue.slice(0, start)}@${inputValue.slice(end)}`;
		...
		window.setTimeout(() => {
			...
			void mentions.updateSuggestions(newText, newCursorPos);
			slashCommands.updateSuggestions(newText, newCursorPos);
		}, 0);
	}, [inputValue, mentions, slashCommands]);

	const measureTextWidth = useCallback(
		(text: string, element: HTMLElement) => {
			if (!text) return 0;
			let canvas = textMeasureCanvasRef.current;
			if (!canvas) {
	...
	const updateDropdownWidth = useCallback(
		(containerEl: HTMLDivElement | null) => {
			...
			if (!label) return;
			const textWidth = measureTextWidth(label, select);
			const minWidth = 36;
			const padding = 12;
			const targetWidth = Math.max(
				minWidth,
				Math.ceil(textWidth + padding),
			);
	...
	const scheduleDropdownWidthUpdate = useCallback(
		(containerEl: HTMLDivElement | null) => {
			if (!containerEl) return;
			window.setTimeout(() => {
				updateDropdownWidth(containerEl);
			}, 0);
		},
		[updateDropdownWidth],
	);
	...
	const setMentionButton = useCallback((el: HTMLButtonElement | null) => {
		if (el) {
			setIcon(el, "at-sign");
		}
	}, []);
	...
	useEffect(() => {
		...
		if (!modeDropdownInstance.current) {
			...
			scheduleDropdownWidthUpdate(containerEl);
	...
	}, [availableModes, scheduleDropdownWidthUpdate]);
	...
	useEffect(() => {
		if (modeDropdownInstance.current && currentModeId) {
			...
			scheduleDropdownWidthUpdate(modeDropdownRef.current);
		}
	}, [currentModeId, scheduleDropdownWidthUpdate]);
	...
	}, [availableModels, scheduleDropdownWidthUpdate]);
	...
	useEffect(() => {
		if (modelDropdownInstance.current && currentModelId) {
			...
			scheduleDropdownWidthUpdate(modelDropdownRef.current);
		}
	}, [currentModelId, scheduleDropdownWidthUpdate]);
